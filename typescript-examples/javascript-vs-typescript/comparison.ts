...
export function runComparisonExamples() {
  ...
  function addJavaScript(a: any, b: any): any {
    return a + b;
  }
  ...
  function greetUserJavaScript(user: any): string {
  ...
  // TypeScript - structure enforcement
  interface ComparisonUser {
    name: string;
    age: number;
  }

  function greetUserTypeScript(user: ComparisonUser): string {
    return `Hello, ${user.name}! You are ${user.age} years old.`;
  }
  ...
  function processNumbersJavaScript(numbers: any[]): any[] {
    return numbers.map((n: any) => n * 2);
  }
  ...
  function processNumbersTypeScript(numbers: number[]): number[] {
  ...
  function configureAppJavaScript(config: any): void {
  ...
  function configureAppTypeScript(config: AppConfig): void {
